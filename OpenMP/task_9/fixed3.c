...
// этого барьера достаточно, чтобы синхронизировать результаты работы тредов
#pragma omp barrier
...
// тут был барьер, но до него не доходили те треды, которые не выполняли код секций
// то есть, если тредов больше 2, то программа зависала
// если его убрать, то треды все равно синхронизируются, так как после секций стоит еще один барьер
