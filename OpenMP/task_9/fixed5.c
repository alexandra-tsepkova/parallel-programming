...
#pragma omp section
            {
                // если потоки берут локи в одинаковом порядке, дедлоков не будет
                // обработка массивов a и b друг от друга не зависит
                // поэтому мы можем поменять порядок обработки массивов, и соответственно, взятия локов
                omp_set_lock(&lockb);
